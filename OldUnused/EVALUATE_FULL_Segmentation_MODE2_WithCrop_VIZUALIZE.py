...
WriteAnn=True
if WriteAnn:
   OutDir=r"/media/sagi/2T/Data_zoo/Viz/"
...
def SplitObjectToParts(Img,ObjectMask,CurrentFolder): # Split  vessel region to materials instance and empyty region instances and classify material
            if not os.path.isdir(CurrentFolder): os.mkdir(CurrentFolder)
...
##==============Collect segments==========================================================================
            while(not ((InsList.shape[0]>120 and (OccupyMask.sum()/ObjectMask.sum())>0.95) or InsList.shape[0]>80)):
    ...
    #==============================Create final annotation map===============================================================================================================================================================
            ...
            while np.max(InstRank)>MinIOUThresh:
                 ...
                 #   Img[0] = Img[0]
                    ...
                    cv2.imwrite(CurrentDir+"/"+str(NumInst)+"_"+str(InstRank[ind])+"_seg.png",(Seg * 6 ).astype(np.uint8) * 230)
                    I=ImgList[0].copy()[..., ::-1]
                    I[:,:,0][Seg>0]=255
                    I[:, :, 1][Seg > 0] = 0
                    I[:, :, 2][Seg > 0] = 255

                    cv2.imwrite(CurrentDir + "/" + str(NumInst) + "_" + str(InstRank[ind]) + "_VIZ.png",I.astype(np.uint8))
                 #   misc.imshow(I.astype(np.uint8))

                #    misc.imshow((ROI[0] * 1 + Seg*6).astype(np.uint8) * 40)

                    cv2.imwrite(CurrentDir + "/" + str(NumInst) + "_" + str(InstRank[ind]) + "_segROI.png",I.astype(np.uint8))
                    #*******************************************************
...
for i in range(4000):
     ...
     #   if (Mask.sum()/ROIMask.sum())<0.01 or Mask.sum()<100: continue#***********************************************************
    ...
    #***********************************************************************************************************************************************
        ...
        #     Img[f, :, :, 1] *= 1-ROIMask[f]
        #   #  Img[f, :, :, 1] *= FullAnnGT
        #     misc.imshow((ROIMask[f] + Mask[f]).astype(np.uint8)*40)
        #     misc.imshow(Img[f])
        # print(ROIMask.shape)
    #***************************************Start loop tht segment the entire objects into parts  by part and pick the segment  best fitted to the target segment in term of IOU***********************************************************************************************************
        ...
        CurrentDir=OutDir+"/"+str(i)+"_"+fname[:-4]
        SegmentationMap=SplitObjectToParts(Img[0], ROIMask[0],CurrentDir)
    ...
    #********************Display segmentation map******************************************************************************
        if WriteAnn:
           ...
           # misc.imshow(VizSegMap)
            ...
            # misc.imshow(Viz)
            nw=1600
            nh=int(nw/w*I1.shape[0])

            cv2.imwrite(CurrentDir + "/ROI.png", ROIMask.astype(np.uint8)*255)
            cv2.imwrite(CurrentDir + "/Img.png", Img[0])
            cv2.imwrite(CurrentDir+"/Label.png", Viz)
            cv2.imwrite(CurrentDir+"/Overlay.jpg", Overlay)
            cv2.imwrite(CurrentDir + "/Overlay2.png", Overlay2)
